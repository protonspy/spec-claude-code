...
`{{.Rules}}/knowledge-base.md`. This skill is when to write one
and how to write one worth reading.

## Nothing asked for

`$ARGUMENTS` is the decision, or the record being superseded. Empty, run
`scc validate` and clear the `adr.*` findings.
